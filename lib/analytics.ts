...
export async function getAnalytics(): Promise<AnalyticsData> {
  try {
    ...
    const [totalUsers, paidUsers, totalNotes, totalReviews, branchStats, subjectStats, recentActivity, paymentStats] =
      await Promise.all([
    ...
    const analytics: AnalyticsData = {
      ...
      paymentStats: {
        ...
        averageOrderValue: paymentData.totalOrders > 0 ? paymentData.totalRevenue / paymentData.totalOrders : 0,
        successRate: paymentData.totalAttempts > 0 ? (paymentData.totalOrders / paymentData.totalAttempts) * 100 : 0,
      },
    }
